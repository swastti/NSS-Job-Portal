...
def delete_user(request,pid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    user = StudentUser.objects.get(id=pid)
    user.delete()
...
def add_job(request):
    ...
    if request.method == 'POST':
        ...
        userr=request.user
        recruiterr= Recruiter.objects.get(user=userr)
        try:
    #         # job =
            Job.objects.create(recruiter=recruiterr,start_date=sd,end_date=ed,title=jt,salary=sal,image=l,decription=des,experience=exp,location=loc,skills=skills,creationdate=date.today())
    #         # job.save()
            error="no"
        except:
            error="yes"
...
#         user = request.user
#         recruiter = Recruiter.objects.get(user=user)
#         job = Job.objects.create(recruiter=recruiter, title=title,startdate=start_date, enddate=end_date, salary=salary, image=image, experience=experience, location=location, skills=skills, description=description, creation_date=date.today())
#         job.save()
#         alert = True
...
def job_list(request):
    ...
    userr=request.user
    recruiterr= Recruiter.objects.get(user=userr)
    job = Job.objects.filter(recruiter=recruiterr)
    d = {'job':job}
...
def applyforjob(request,pid):
    ...
    if job.end_date < date1:
        ...
        #
        Apply.objects.create(student=student,job=job,applydate=date.today())
        error="done"
